...
QStringList BH::Qshow() const {
    QStringList line;
    line << QString::fromStdString(hid) << QString::fromStdString(rid) << QString::fromStdString(rname)
         << QString::fromStdString(bid) << QString::fromStdString(bname) << QString::fromStdString(btime)
         << QString::fromStdString(rtime) << QString::fromStdString(note);
    return line;
}
...
QTableModel* HL::Qshow() const {
    QTableModel *model = new QTableModel(QTableModel::History);

    QVector<QStringList> data;
    for(int i=0;i<hl.size();i++){
        data.append(hl[i].Qshow());
    }
...
QTableModel* HL::QschHistory(const string &s) const {
    QTableModel *model = new QTableModel(QTableModel::History);
